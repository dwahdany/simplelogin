...
- **URL:** https://simplelogin.dariush-5fc.workers.dev (Cloudflare account
  `Dariush@wahdany.de`, worker name `simplelogin`).
- **Alias/email domain:** `sl-test.wahdany.eu` — a **subdomain** of the
  `wahdany.eu` zone. Email Routing + Email Sending are enabled on the
  **subdomain only**; the apex `wahdany.eu` MX still points at Protonmail and
  must not be touched. A zone catch-all rule routes `*@sl-test.wahdany.eu` to
...
- **Test account:** `dariushwahdany@gmail.com` (change the password via the
  dashboard). Test alias created: `slug_eagle137@sl-test.wahdany.eu`. The
