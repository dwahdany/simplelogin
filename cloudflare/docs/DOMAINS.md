...
2. **Email Routing enabled** on the domain (auto-creates + locks the MX
   ...
   (like the live `sl-test.wahdany.eu` under the `wahdany.eu` zone) are
...
2. Fields:
   ...
   - **Redirect URL**: `<URL var>/dashboard/cloudflare/callback`, e.g.
     `https://simplelogin.dariush-5fc.workers.dev/dashboard/cloudflare/callback`.
...
- The server refuses domains that overlap the deployment's own mail
  ...
  `foo.wahdany.eu` next to `sl-test.wahdany.eu`) — token zone-scoping
...
- **Domain Connect: not usable here.** Cloudflare *is* a Domain Connect DNS
  provider, sync-only: `dig TXT _domainconnect.wahdany.eu` returns
